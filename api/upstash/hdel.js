/**
 * Vercel API Route: Upstash HDEL command
 * Securely handles Redis HDEL operations using environment variables
 */
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const response = await fetch(`${upstashUrl}/hdel`, {
            method: 'POST',
            headers: {
        ...
        console.error('Upstash HDEL error:', error);
        return res.status(500).json({
            success: false,
